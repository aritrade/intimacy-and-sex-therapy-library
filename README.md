...
[![Watch the primer — a 4-minute cinematic tour of the Intimacy & Sex Therapy Library](https://raw.githubusercontent.com/aritrade/intimacy-and-sex-therapy-library/main/public/marketing/primer-preview.gif)](https://github.com/aritrade/intimacy-and-sex-therapy-library/blob/main/public/marketing/primer-readme.mp4)

<p align="center"><em>The looping preview above plays inline (no sound). <strong>Click it</strong> for the full film with narration in GitHub's player &middot; or get <a href="https://raw.githubusercontent.com/aritrade/intimacy-and-sex-therapy-library/main/public/marketing/primer.mp4">the HD&nbsp;MP4</a> &middot; or experience it on the <a href="https://intimacy-and-sex-therapy-library.vercel.app">live site</a>.</em></p>
...
The full generate → render → publish loop runs end-to-end without any
local machine — generation, rendering, and publishing all run on GitHub
Actions; Vercel handles only the catalog-sync and metrics crons.

```
05:00 IST  daily-generate (GitHub Actions cron at 23:30 UTC)
           ├─ Picks 5 briefs from CONTENT_BRIEFS (3 short EN/Hinglish + 2 long EN)
           ├─ Calls Groq Llama-3 to write each script — sequential + spaced so
           │  the batch stays under Groq's tokens-per-minute limit
           └─ Inserts each as `script_draft`
...
Every 6h   cron-auth-canary (GitHub Actions)
           └─ Probes the cron auth with a real and a bogus CRON_SECRET and opens
              a GitHub issue if the secret drifts between GitHub and Vercel
```

> Generation moved off the Vercel cron in Phase 18 — the Hobby plan's 60s
> function cap was killing long-form essays mid-run. GitHub Actions has no
> such cap, and serialising the Groq calls clears the rate limit. The
> `/api/cron/daily-generate` route still exists for manual Bearer-authed
> triggers; it's just no longer the scheduled generator. The generation logic
> is shared between the route and the GH script via
> [`lib/social/daily-generate-core.ts`](lib/social/daily-generate-core.ts).

Operator's daily ritual (5–10 min, **any browser, any device**):

1. Open `/admin/queue` → 5 cards (3 reels + 2 long-form) already have video previews attached.
...
| `/api/cron/daily-generate`                     | cron    | Manual Bearer trigger only — scheduled generation runs via GH Actions `daily-generate.yml` (5 fresh `script_draft` rows) |
...
- **Vercel cron** — daily content sync (03:00 IST) and weekly metrics
  poll (Mon 11:30 IST). See [`vercel.json`](vercel.json). (Daily script
  generation moved to GitHub Actions in Phase 18.)
- **GitHub Actions cron** —
  [`daily-generate.yml`](.github/workflows/daily-generate.yml) daily at
  23:30 UTC (runs `scripts/daily-generate.ts` against Neon + Groq;
  sequential + spaced to stay under the TPM limit),
...
- **Cron auth canary + failure alerts** —
  [`cron-auth-canary.yml`](.github/workflows/cron-auth-canary.yml) probes
  the cron auth every 6h (real + bogus `CRON_SECRET`) to catch secret
  drift between GitHub and Vercel. On failure, it and `publish-due` /
  `daily-generate` open a deduped GitHub issue via the shared
  [`pipeline-alert`](.github/actions/pipeline-alert/action.yml) action, so
  a silently-broken cron surfaces in notifications instead of the Actions tab.
...
```
                     ┌────────────────────────┐
   Public users ───▶ │  Next.js (Vercel/bom1) │ ◀── Vercel Cron (2 jobs)
                     │  - middleware role gate│      (daily-content-sync,
                     │  - server components   │       post-metrics-poll)
                     └─────────┬──────────────┘
                               │
            ┌──────────────────┼──────────────────┐
            ▼                  ▼                  ▼
   ...
   GitHub Actions ───▶  daily-generate (daily) ─▶  Neon (script_draft rows)
                        render-due (hourly)    ─▶  Vercel Blob (MP4)
                        publish-due (hourly)   ─▶  IG Graph API
                        cron-auth-canary (6h)      YT Data API v3
                                                  FB Page Reels API
```
...
lib/
  ...
  auth/                  Auth.js config, role helpers, page + API guards
                         (Basic-auth fallback honoured everywhere — ADR-0001)
  ...
  social/                script gen, render pipeline, TTS, publishers
    daily-generate-core.ts  shared brief-pick + generate loop used by the
                            Vercel route and the GH Actions script
...
.github/workflows/       ci, eval-nightly, lighthouse, daily-generate,
                         render-due, publish-due, cron-auth-canary,
                         avatar-render
.github/actions/         pipeline-alert (deduped issue-on-failure)
scripts/                 seed-*, ingest, eval, render-draft, render-due,
                         daily-generate, render-brand-assets, preflight, …
```
...
- **Phase 17** **Mac-free autonomous operation**: GitHub
...
- **Phase 18 (current)** **Generation off Vercel + pipeline resilience**:
  daily script generation moved from the Vercel Hobby cron (60s cap was
  truncating long-form essays) to a GitHub Actions cron running
  `scripts/daily-generate.ts`, with Groq calls serialised + spaced to stay
  under the tokens-per-minute limit. Daily mix is now 3 short + 2 long = 5
  drafts. Cron auth hardened with a 6-hourly `cron-auth-canary` and a shared
  `pipeline-alert` action that opens a deduped GitHub issue when any cron
